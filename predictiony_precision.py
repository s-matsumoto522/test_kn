...
#datファイルからデータを読み取る
axisx, axisy = np.loadtxt("./chk_predictiony_precision.dat", comments='!', unpack=True)
#x, yの値の設定
x = axisx
y = axisy
...
# 3. データを渡してプロット
line1, = ax.plot(x, y, label = 'predictiony_precision', marker='.')
# 4. グラフタイトル, ラベル付け等
...
ax.set_title("check the predictiony precision")
ax.legend()
# 5. グラフを描画
plt.grid(which="both")
plt.show()